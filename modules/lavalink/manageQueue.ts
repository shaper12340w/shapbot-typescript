...
export class Queue{
    ...
    constructor(channelId:string,guildId:string,voiceChannelId:string){
        ...
        this.data = {
            option:{
                ...
                playShuffle:0,
                playRecommend:0,
                playVolume:Number(client.serverProperty.get(guildId)!!.player.volume) ?? 100,
                playNextOption:true,
            },
            playList:<playlistOption[]>[],
            rowPlaylist:<Track[]>[],
    ...
    private async getTrack():Promise<Track|null>{
        return await this.data.player!!.node.rest.decode(this.data.player!!.track!!);
    }
    ...
    private async recommendPlay():Promise<APIEmbed|undefined> {
        try{
    ...
    public async playing(){
        ...
        // @ts-ignore
        const videoLength = (<Track>await this.getTrack()).length;
