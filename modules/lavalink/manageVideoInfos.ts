...
export class ManageVideoInfos extends CompareString{
    constructor(){
        super()
    }
...
export class getVideoChapters{
    public static async getChapters(url:string):Promise<playdl.VideoOptions["chapters"]>{
        const getInfo = await playdl.video_basic_info(url);
        const chapters = getInfo.video_details.chapters;
        return chapters;
    }
}