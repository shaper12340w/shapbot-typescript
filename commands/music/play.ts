import { SlashCommandBuilder,CommandInteraction,CommandInteractionOptionResolver,VoiceBasedChannel, EmbedBuilder, MessageReplyOptions } from 'discord.js';
import { Command } from '../../structures/Command'
import { Queue, embType, returnType } from '../../modules/lavalink/manageQueue';
...
interface ReturnEmbedData extends MessageReplyOptions { content:string; embeds:embType[] };
...
async function selectMessage(searchResult: Track[], interaction: CommandInteraction):Promise<void> {
    ...
    if(searchResult.length === 1){
        ...
        .catch(async (err:unknown)=>{
            const result = await sendAddMessage(0);
            interaction.editReply({ embeds: [makeEmbed("선택하지 않아 자동으로 1번이 선택되었습니다","`"+result.embeds[0].title+"`","yellow",":grey_exclamation:")]})
            interaction.channel!!.send(result);        
        })
    }
}
