...
import { createButtonSet, createModal, createStringSelectMenuBuilder } from '../../modules/common/interactions';
...
export default new Command({
	...
	async execute(interaction:CommandInteraction) {
        ...
        if(!queue.get(interaction.guildId!!)){
            ...
            const nowPlayList:playlistOption[] = (<playlistOption[]>JSON.parse(JSON.stringify(queue.get(interaction.guildId!!)!!.data.playList))).map(e=>{e.status=1;return e}); //현재 큐의 재생목록 
            ...
            const button = createButtonSet(interaction.id,[
                {
                    label: '저장',
                    style: 1,
