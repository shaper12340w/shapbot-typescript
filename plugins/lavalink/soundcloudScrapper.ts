...
const Constants = config.regex.soundcloud
...
export class SoundcloudScrapper {
    ...
    static async findRelated(track_id: string) {
        if (!config.key.soundcloud) config.key.soundcloud = await SoundcloudScrapper.keygen() as string;
        Logger.debug(config.key.soundcloud);
        const url = `https://api-v2.soundcloud.com/tracks/${track_id}/related`;
        const params = {
            client_id: config.key.soundcloud,
            limit: 10,
            offset: 0,
